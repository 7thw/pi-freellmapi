...
function firstDeltaTimeoutMessage(timeoutMs: number, attempts: number): string {
	return `FREELLMAPI stream timed out: no text, thinking, or tool delta arrived within ${timeoutMs}ms after response headers on ${attempts} attempts.`;
}

export function resolveFirstDeltaTimeoutMs(): number {
	const raw = process.env.FREELLMAPI_STREAM_FIRST_DELTA_TIMEOUT_MS;
...
export function streamFreellmapiOpenAICompletions(
	model: Model<Api>,
	context: Context,
	options: SimpleStreamOptions | undefined,
	firstDeltaTimeoutMs: number,
	...
	pickerDebug("stream_simple_entered", {
		...
		model: model.id,
		isFreellmapi: model.provider === PROVIDER_KEY,
